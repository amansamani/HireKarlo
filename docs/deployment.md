...
The application and backend are one Next.js service. Vercel hosts the web/functions; PostgreSQL stores tenant and work queues; SMTP delivers email, Cloudinary stores new private resumes, Gemini optionally reviews them and Google optionally creates calendar events. Razorpay/Stripe configuration is environment-specific. Separate environment credentials and databases are required. Follow [Razorpay setup](razorpay-setup.md) for the new INR subscription integration; existing Stripe accounts require an operator-managed migration before changing provider.
...
| `BILLING_PROVIDER`, `RAZORPAY_MODE` | Select `razorpay` and `test` initially; live uses separate credentials and database |
| `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`, `RAZORPAY_WEBHOOK_SECRET` | Private API credentials plus a distinct webhook secret |
| `RAZORPAY_PLAN_{STARTER,GROWTH,AGENCY}_INR` | Three matching INR monthly plans; `npm run razorpay:setup` prepares Test Mode plans |
...
3. Verify a current live backup and restore plan. Run `npm run db:preflight` with the intended live connection privately. A previous preflight result does not replace a release-window check.
4. Apply all pending migrations, including `20260918000000_launch_workflows`, `20260918010000_razorpay_billing` and `20260918011000_billing_shared_customer`, in a controlled release window with `npm run db:deploy` against the intended database. Deploy code that uses the new columns/tables only after migration succeeds. Do not use `migrate reset` or `db push` on live data.
...
See [18 September launch fixes and acceptance gates](launch-fixes-2026-09-18.md) for the current code changes, migration order and unresolved provider/operational work.
