...
export default async function ClientsPage({ searchParams }: { searchParams: Promise<{ error?: string; page?: string }> }) {
  ...
  return <section className="mx-auto max-w-6xl"><h1 className="text-3xl font-bold">Agency clients</h1><p className="mt-3 text-muted-foreground">Keep client contacts and connect their open roles to your hiring pipeline.</p>
    ...
    <form action={createAgencyClientAction} className="my-7 rounded-2xl border border-border bg-card/70 p-6"><h2 className="text-xl font-semibold">Add a client</h2><div className="mt-4 grid gap-4 md:grid-cols-2"><label>Company name<input className={inputClass} name="name" required minLength={2} maxLength={150}/></label><label>Contact email<input className={inputClass} name="contactEmail" type="email"/></label></div><label className="mt-4 block">Notes<textarea className={inputClass} name="notes" maxLength={3000}/></label><SubmitButton pendingLabel="Adding client…" className="mt-5 rounded-lg bg-primary px-5 py-3 text-primary-foreground">Add client</SubmitButton></form>
    <section><h2 className="text-xl font-semibold">Assign active jobs</h2><div className="mt-4 space-y-3">{jobs.map(job => <form key={job.id} action={assignJobClientAction} className="flex flex-wrap items-center gap-3 rounded-2xl border border-border bg-card/50 p-5"><Link className="flex-1 font-semibold" href={`/dashboard/jobs/${job.id}`}>{job.title}</Link><input type="hidden" name="jobId" value={job.id}/><select aria-label={`Client for ${job.title}`} name="clientId" defaultValue={job.clientId ?? ""} className="max-w-full rounded-lg border border-border bg-background p-3"><option value="">Internal company role</option>{choices.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select><SubmitButton className="rounded border border-border px-4 py-2">Save</SubmitButton></form>)}{!jobs.length && <p className="text-muted-foreground">Create a job to assign it to a client.</p>}</div></section>
    <div className="mt-8 grid gap-4 md:grid-cols-2">{clients.slice(0,25).map(client => <article key={client.id} className="rounded-2xl border border-border bg-card/70 p-6"><h2 className="text-xl font-bold">{client.name}</h2><p className="mt-2 text-sm text-muted-foreground">{client.contactEmail ?? "No contact email"} · {client._count.jobs} jobs</p>{client.notes && <p className="mt-3 whitespace-pre-wrap text-sm">{client.notes}</p>}<ul className="mt-4 space-y-2">{client.jobs.map(job => <li key={job.id}><Link className="text-primary" href={`/dashboard/jobs/${job.id}`}>{job.title}</Link> <span className="text-xs text-muted-foreground">{job.status}</span></li>)}</ul></article>)}</div>
