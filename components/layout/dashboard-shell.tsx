...
export default function DashboardShell({ children, role }: { children: React.ReactNode; role: string | null }) {
  ...
  return (
    <div className="flex h-dvh w-screen overflow-hidden bg-background text-foreground">
      ...
      <Sidebar mobileOpen={mobileNavOpen} onClose={closeNavigation} role={role} />

      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <Header onMenuClick={() => setMobileNavOpen(true)} />
        <main
          id="main-content"
          className="workspace-content flex-1 overflow-y-auto p-4 selection:bg-primary/20 sm:p-6 lg:p-8"
        >
          {children}
        </main>
      </div>
